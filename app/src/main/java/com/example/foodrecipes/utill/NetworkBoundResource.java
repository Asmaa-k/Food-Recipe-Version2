...
public abstract class NetworkBoundResource<CacheObject, RequestObject> {
    ...
    /*hold the cache, coz we wanna retrive data from cache
  ...
  public NetworkBoundResource(AppExecutor appExecutor) {
    ...
    private void init() {
        ...
        //Just if data loadFromDb() then onChanged dbSource
        //we decide wheater or not, send the ui
    ...
    private void fetchFromNetwork(final LiveData<CacheObject> dbSource) {
        ...
        results.addSource(dbSource, new Observer<CacheObject>() {
            @Override
            public void onChanged(@Nullable CacheObject cacheObject) {
                setValue(Resource.loading(cacheObject));//Just To View The Cache
            }
        });
                    ...
                    appExecutor.diskIO().execute(new Runnable() {
                        ...
                        public void run() {
                            ...
                            appExecutor.mainThread().execute(new Runnable() {
                                //I should observe that I called background thread before I called the main thread(.diskIO().execute
                                // , then I goes to appExecutor.mainThread().execute) otherwise the app will crush
                                @Override
                                public void run() {
